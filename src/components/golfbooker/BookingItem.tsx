...
export function BookingItem({ booking, onEdit }: Props) {
  ...
  // Auto-close log listener when collapsed
  const toggleLogs = () => {
    ...
    <div className="border border-border rounded-xl p-4 mb-3 last:mb-0 bg-surface">
      ...
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-semibold text-sm">{targetDate}</span>
        ...
        <div className="flex gap-1.5">
          {canEdit && (
            <Button size="sm" variant="secondary" onClick={() => onEdit(booking)}>
              Edit
            </Button>
          )}
          {canEdit && (
      ...
      {(booking.foursomes ?? []).map((fs, i) => (
        <div
          key={i}
          className="py-1.5 border-b border-border last:border-b-0 text-sm"
        >
          <span className="font-medium">{fs.accountOwner}</span>
          <span className="text-muted"> ({fs.account}) at </span>
          <span className="font-medium">{fs.targetTime}</span>
      ...
      {showLogs && (
        <div className="mt-3">
          <button
            type="button"
            onClick={toggleLogs}
            className="text-xs text-accent hover:underline bg-none border-none cursor-pointer font-[inherit] p-0"
          >
            {logsOpen ? "▼ Hide Logs" : "▶ Show Logs"}
          </button>
          {logsOpen && (
            <div className="mt-2 bg-bg border border-border rounded-lg max-h-48 overflow-y-auto font-mono text-[0.7rem] leading-relaxed p-2">
              {logsLoading && (
                <div className="text-muted">Loading logs…</div>
              )}
              {!logsLoading && logs.length === 0 && (
                <div className="text-muted">No logs yet — waiting for execution…</div>
              )}
              {logs.map((log, i) => {
